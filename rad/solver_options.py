...
class setup_solver_group():
	...
	def __init__(self):
		...
		pseudospherical = option(
			...
			tokens=addSetting(name='Input.rte.pseudospherical', setting=1),
			parents=['rte_solver'],
			speaker='rte_solver',
			enable_values=("disort","twostr",)
		)

		disort_spherical_albedo = option(
			name='disort_spherical_albedo',
			group='solver',
			helpstr='Calculate spherical albedo.',
			documentation=documentation['disort_spherical_albedo'],
			tokens=[addSetting(name='Input.rte.ibcnd', setting=1),
				  addSetting(name='Input.rte.nphi', setting=0)],
			parents=['rte_solver'],
			speaker='rte_solver',
		...
		isotropic_source_toa = option(
			...
			tokens=addSetting(name='Input.rte.fisot', setting=1),
			gui_inputs=(FloatInput(name='isotropic_source_toa'),),
			parents=['rte_solver'],
			speaker='rte_solver',
			enable_values=("disort",
				      "twostr",)
		...
		raman = option(
			...
			tokens=[addSetting(name='Input.raman', setting='RAMAN_CALC'),
				addSetting(name='Input.ck_scheme', setting='CK_RAMAN'),
				addSetting(name='Input.rte.solver', setting='SOLVER_DISORT'),
				addSetting(name='Input.processing', setting='PROCESS_RAMAN'),
				addLogical(name='Input.raman_original',logicals=['original'],optional=True) ],
			gui_inputs=(ListInput(name='raman_original',valid_range=['original'],optional=True),),
			parents=['rte_solver'],
			speaker='rte_solver',
			enable_values=("disort",),
		)

		number_of_streams = option(
			name='number_of_streams',
			group='solver',
			helpstr='Number of streams.',
			documentation=documentation['number_of_streams'],
			gui_inputs=(IntegerInput(name='number_of_streams'),),
			tokens=addToken(name='Input.rte.nstr', datatype=int, default=6),
			parents=['rte_solver'],
			speaker='rte_solver',
		...
		polradtran = option(
			...
			tokens=[addLogical(name='id', logicals=['aziorder','nstokes','src_code'], setting='POLRADTRAN_'),
				addToken(name='Input.rte.polradtran[id]', datatype=int) ],
			speaker='rte_solver',
		...
		polradtran_quad_type = option(
			name='polradtran_quad_type',
			group='solver',
			helpstr='Type of quadrature used',
			documentation=documentation['polradtran_quad_type'],
			tokens=addToken(name='Input.rte.pol_quad_type', valid_range=['G', 'D', 'L', 'E'], datatype=str),
			parents=['rte_solver'],
			speaker="rte_solver",
			enable_values=("polradtran",),
			showInGui=False,
		...
		polradtran_max_delta_tau = option(
			...
			tokens=addToken(name='Input.rte.pol_max_delta_tau', datatype=Double, default=1e-5 ),
			parents=['rte_solver'],
			speaker="rte_solver",
			enable_values=("polradtran",),
			showInGui=False,
...
#no_sdisort			gui_inputs=(IntegerListInput(name='Input.rte.nrefrac', default=0, valid_range=[0, 1, 2], optional=False),),
...
#no_sdisort		)
		...
		sdisort = option(
			...
			tokens=[addLogical(name='id', logicals=['ichapman','nscat','nrefrac'],setting='SDISORT_'),
				addToken(name='Input.rte.sdisort[id]', datatype=int) ],
			speaker='rte_solver',
		...
		sos_nscat = option(
			...
			tokens=	addToken(name='Input.rte.sos_nscat', datatype=int) ,
			speaker='rte_solver',
			enable_values=('sos',),
			showInGui=False,
                        developer=True
		...
		deltam = option(
			...
			gui_inputs=(ListInput(name='Input.rte.deltam', valid_range=['on', 'off'], optional=False, default='on'),),
			tokens=addLogical(name='Input.rte.deltam', logicals=['on','off'], setting='SWITCH_', default='SWITCH_ON'),
			parents=['rte_solver'],
			speaker="rte_solver",
			enable_values=("disort", "twostr",)
		)
		...
		ipa_3d = option(
			...
			tokens=addSetting(name='Input.ipa3d',setting=1),
			threedmystic =True,
			parents=['uvspec'],
			non_parents=['tipa',],
			speaker='rte_solver',
		...
		mc_tenstream = option(
			...
			tokens=addSetting(name='Input.rte.mc.tenstream', setting=1,default=0),
			parents=['uvspec'],
			speaker='rte_solver',
		...
		mc_tipa = option(
			...
			tokens=[addSetting(name='Input.rte.mc.ipa', setting=1),
				addLogical(name='Input.rte.mc.tipa', logicals=['dirdiff','dir3d'], setting='TIPA_') ],
			parents=['uvspec'],
			non_parents=['mc_ipa',],
			speaker='rte_solver',
			enable_values=("mystic",),
		...
		tipa = option(
			...
			tokens=addLogical(name='Input.tipa',logicals=['dirdiff','dir'],setting='TIPA_'),
			parents=['uvspec'],
			non_parents=['ipa_3d',],
			speaker='rte_solver',
		...
		sslidar = option(
			...
			tokens= [ addLogical(name='id',logicals=['area','E0','eff','position','range'],setting='SSLIDAR_'),
				addToken(name='Input.sslidar[id]', datatype=float) ],
			gui_inputs= (ListInput(name='id', valid_range=['area','e0','eff','position','range']),FloatInput(name='Input.sslidar[id]'),),
			parents=['uvspec'],
			speaker='rte_solver',
		...
		sslidar_nranges = option(
			...
			tokens= [ addToken(name='Input.sslidar_nranges', datatype=int) ],
			gui_inputs= (IntegerInput(name='Input.sslidar_nranges'),),
			parents=['uvspec'],
			speaker='rte_solver',
			enable_values=("sslidar",),
		)

		sslidar_polarisation = option(
			...
			tokens= addSetting(name='Input.sslidar_polarisation', setting=1),
			parents=['uvspec'],
			speaker='rte_solver',
			enable_values=("sslidar",),
		)

                tzs_cloud_top_height = not_yet_lex2py_option(
