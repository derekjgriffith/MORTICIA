...
class GreyscaleLambertianBeachballTarget(Target):
    ...
    """
    def __init__(self, diam= 1.0, n_sector_pairs= 12, min_refl= 0.0, max_refl= 1.0, n_pix= 128, anti_aliasing= 1):
        """

        :param diam:
        :param n_sector_pairs:
        :param normal_vector
        :param min_refl
        :param max_refl
        :param n_pix
        :param anti_aliasing is set to 1 by default, this means there is no oversampling. should be an integer equal 
                to how many times more resolution should the target have in the image plane relative to n_pix
        :return:
        """
    ...
    def get_target(self):
        ...
        if self.anti_aliasing > 1.0:
            ...
            siemans_star = sg.convolve2d(siemans_star, the_filter, mode = 'same')
            mask = sg.convolve2d(mask, the_filter, mode = 'same') 
