...
class setup_output_group():
	...
	def __init__(self):
		...
		quiet = option(
			...
			tokens=addSetting(name='Input.quiet', setting=1, default=0),
			parents=['uvspec'],
			non_parents=['verbose'],
		)
		...
		verbose = option(
			...
			tokens=addSetting(name='Input.verbose', setting=1, default=0),
			parents=['uvspec'],
			non_parents=['quiet'],
		)

		write_ext_to_file = option(
			name='write_ext_to_file',
			group='output',
			helpstr='If specified abundances of informative messages are output to stderr.',
			documentation=documentation['write_ext_to_file'],
			tokens=addSetting(name='Input.write_ext_to_file', setting=1, default=0),
			parents=['uvspec'],
			islidar=True,
		)

		test_optical_properties = option(
			...
			tokens=addSetting(name='Input.test_optical_properties', setting=1, default=0),
			parents=['uvspec'],
			developer=True,
		)
		...
		output_process = option(
			...
			gui_inputs=(ListInput(name='Input.output_unit_processing', valid_range=['integrate', 'sum', 'rgbnorm', 'rgb_norm', 'rgb', 'per_nm', 'per_cm-1', 'per_band'], optional=False),),
		...
		output_quantity = option(
			...
			tokens=addLogical(name='Input.calibration', logicals=['reflectivity','transmittance','brightness'], setting='OUTCAL_' ),
			parents=['uvspec'],
		)
		...
		heating_rate = option(
			...
			tokens = [ addSetting(name='Input.heating', setting='HEAT_LAYER_CD'),
				   addLogical(name='Input.heating', logicals=['none','local','layer_fd','layer_cd'], setting='HEAT_', optional=True) ] ,
			parents=['uvspec'],
		)

		write_output_as_netcdf = option(
			name='write_output_as_netcdf',
			group='output',
			helpstr='Output is written into netcdf file.',
			documentation=documentation['write_output_as_netcdf'],
			tokens=addSetting(name='Input.write_output_as_netcdf', setting=1),
			parents=['uvspec'],
			islidar=True,
			showInGui=False,
		)

		slit_function_file = option(
		...
		spline = option(
			...
			gui_inputs=(FloatInput(name='Input.spline_lambda_0', valid_range=[0, 1000000.0]), FloatInput(name='Input.spline_lambda_1', valid_range=[0, 1000000.0]), FloatInput(name='Input.spline_lambda_step', valid_range=[0, 1000000.0]),),
			tokens = [ addToken(name='Input.spline_lambda_0', datatype=float, valid_range=[0,1e6] ), #TODO:Valid_range from option wavelength.
				addToken(name='Input.spline_lambda_1', datatype=float, valid_range=[0,1e6] ),
				addToken(name='Input.spline_lambda_step', datatype=float, valid_range=[0,1e6]),
		...
		pressure_out = not_yet_lex2py_option(
			name='pressure_out',
			group='output',
		...
		mc_backward_output = option(
			...
			tokens=[ addLogical(name='Input.rte.mc.backward.output', logicals=['edir','edn','eup','fdir','fdn','fup','act','abs','emis','heat'], setting='MCBACKWARD_', gui_name='output' ),	
		...
		mc_surfaceparallel = option(
			...
			tokens=addSetting(name='Input.rte.mc.surfaceparallel', setting=1),
			parents=['uvspec'],
			speaker='rte_solver',
			enable_values=("mystic","montecarlo"),
			threedmystic =True,
		...
		mc_jacobian = option(
			...
			tokens=addSetting(name='Input.rte.mc.jacobian', setting=1),
			parents=['uvspec'],
