...
class setup_cloud_group():
	...
	def __init__(self):
		...
		profile_file = option( 	#TODO: option 3D only ifthreedmystic
			name='profile_file',
			group='profile',
...
#			gui_inputs=(ListInput(name='Input.caoth[isp].properties', valid_range=['hu', 'echam4', 'mie', 'fu', 'yang', 'key', 'baum_hufit', 'baum', 'hey', 'ic_mie'], optional=False), ListInput(name='Input.caoth[isp].interpolate', valid_range=['','interpolate'], optional=True),),
		...
		profile_modify = option(	#TODO: valid_ranges for GUI!!
			name='profile_modify',
			group='profile',
			helpstr='Modify profile properties.',
			documentation=documentation['profile_modify'],
#			gui_inputs=(ListInput(name='id1', valid_range=['gg', 'ssa', 'tau', 'tau550'], optional=False), ListInput(name='id2', valid_range=['set', 'scale'], optional=False), FloatInput(name='Input.caoth[isp].modify[id1][id2]'),),
			tokens = [ addToken( name='isp', datatype=CaothType ),
