...
class setup_geometry_group():
	...
	def __init__(self):
		...
		phi = option(
			...
			tokens = [ addToken(name='Input.rte.phi', datatype=SignedFloats),
				addSetting(name='Input.rte.nphi', setting='ntokens'),
				addSetting(name='Input.rte.maxphi', setting='ntokens') ],
			parents=['uvspec'],
		)
		...
		umu = option(
			...
			tokens = [ addToken(name='Input.rte.umu', datatype=SignedFloats),
				addSetting(name='Input.rte.numu', setting='ntokens'),
				addSetting(name='Input.rte.maxumu', setting='ntokens') ],
			parents=['uvspec'],
			non_parents=[],
			childs=[]
		)
		...
		longitude = option(
			...
			tokens = [ addLogical(name='Input.lon_signum',logicals=['e','w'],setting='LONGITUDE_SIGNUM_'),
				...
				addToken(name='Input.lon_minutes', datatype=float, optional=True),
				addToken(name='Input.lon_seconds', datatype=float, optional=True) ],
			parents=['uvspec'],
			extra_dependencies=['latitude','longitude'],
			showInGui=False,
		)
		...
		mc_sample_grid = option(
			...
			tokens= [ addToken(name="Input.rte.mc.Nx_sample", datatype=int),
				  ...
				  addToken(name="Input.rte.mc.dx_sample", datatype=Double, optional=True),
				  addToken(name="Input.rte.mc.dy_sample", datatype=Double, optional=True) ],
			parents=['uvspec'],
			speaker='rte_solver',
			enable_values=("mystic","montecarlo"),
			threedmystic =True,
		...
		mc_spherical = option(
			...
			tokens=[addLogical(name='id', logicals=['1D','3D'], setting='DIM_'),
				addSetting(name='Input.rte.mc.spherical[id]', setting=1) ],
			gui_inputs=(ListInput(name='dimension', valid_range=['1d']),),
			parents='mc_backward',
		...
		mc_spherical3D_scene = option(
			...
			tokens=[addSetting(name='Input.rte.mc.spherical3D_scene', setting=1),
				...
				addToken(name='Input.rte.mc.spherical3D_scene_lon_max', datatype=float, gui_name='LON_E'),
				addToken(name='Input.rte.mc.spherical3D_scene_lat_min', datatype=float, gui_name='LAT_S'),
				addToken(name='Input.rte.mc.spherical3D_scene_lat_max', datatype=float, gui_name='LAT_N') ],
		...
		mc_satellite_view = option(
			...
			tokens=[addSetting(name='Input.rte.mc.pan_angles', setting='calloc (4, sizeof(double))'),
				...
				addToken(name='Input.rte.mc.pan_angles[1]', datatype=Double),
				addToken(name='Input.rte.mc.pan_angles[2]', datatype=Double),
				addToken(name='Input.rte.mc.pan_angles[3]', datatype=Double),
				addSetting(name='Input.rte.mc.panorama', setting='PAN_MODE_SATELLITE'),
				addSetting(name='Input.rte.mc.allocate_umu_and_phi', setting=True) ],
			parents=['mc_backward'],
		...
		mc_satellite_position = option(
			...
			tokens=[addSetting(name='Input.rte.mc.sensorposition', setting='MC_SENSORPOSITION_SPHERICAL'),
				...
				addToken(name='Input.rte.mc.sensorposition_lat', datatype=float),
				addToken(name='Input.rte.mc.sensorposition_alt', datatype=float) ],
		...
		mc_sun_position = option(
			...
			tokens=[addSetting(name='Input.atm.sza_source', setting='SZA_DIRECT_INPUT'),
				...
				addToken(name='Input.atm.sza_spher', datatype=float) ],
			islidar=True,
			speaker='mc_sperical',
			enable_values=('3D',),
			showInGui=False,
		...
		mc_sensordirection = option(
			...
			tokens=[ addToken(name="Input.rte.mc.sensordirection_dx", datatype=float, gui_name='dx'),
				...
				addToken(name="Input.rte.mc.sensordirection_dz", datatype=float, gui_name='dz'),
				addSetting(name="Input.rte.mc.sensordirection", setting=1)  ],
			parents=['mc_backward'],
			speaker='rte_solver',
			enable_values=("mystic","montecarlo"),
			threedmystic =True,
		...
		mc_sensorposition = option(
			...
			tokens=[  addToken(name="Input.rte.mc.sensorposition_x", datatype=float, gui_name='x'),
				  ...
				  addToken(name="Input.rte.mc.sensorposition_z", datatype=float, gui_name='z'),
		...
		mc_panorama_view = option(
			...
			tokens=[addSetting(name='Input.rte.mc.pan_angles', setting='calloc (4, sizeof(double))'),
				...
				addToken(name='Input.rte.mc.pan_angles[2]', datatype=Double),
				addToken(name='Input.rte.mc.pan_angles[3]', datatype=Double),
				addSetting(name='Input.rte.mc.panorama', setting='PAN_MODE_CAMERA'),
				addSetting(name='Input.rte.mc.allocate_umu_and_phi', setting=True) ],
			parents=['mc_backward'],
		...
		mc_panorama = option(
			...
			tokens=[addLogical(name='id',logicals=['distr_photons_over_pixel','no_pixel','quicklook','with_direct_rad','weight_with_cos','circumsolar_var_red'],setting='PAN_FLAG_'),
		...
		mc_blitz_position = option(
			...
			tokens=[addSetting(name='Input.rte.mc.blitz_position', setting='calloc (6, sizeof(double))'),
				...
				addToken(name='Input.rte.mc.blitz_position[1]', datatype=Double),
				addToken(name='Input.rte.mc.blitz_position[2]', datatype=Double),
				addToken(name='Input.rte.mc.blitz_position[3]', datatype=Double),
				addToken(name='Input.rte.mc.blitz_position[4]', datatype=Double),
				addToken(name='Input.rte.mc.blitz_position[5]', datatype=Double),
				addSetting(name='Input.source', setting='SRC_BLITZ'),
				addSetting(name='Input.rte.mc.allocate_umu_and_phi', setting=True) ],
			parents=['mc_forward'],
			speaker='rte_solver',
			enable_values=("mystic","montecarlo"),
